...
class DiagnosableGMM(nn.Module):
    def __init__(
        ...
        if init_means is None:
            init_means = torch.randn(num_components, input_dim)
        self.q_mu = nn.Parameter(init_means.float())

        if init_logvar is None:
            init_logvar = torch.ones(num_components, input_dim) * -1.0
        self.q_logvar = nn.Parameter(init_logvar.float())

        if init_pi is None:
            init_pi = torch.full((num_components,), 1.0 / num_components)
    ...
    def loss_components(self, X: torch.Tensor, beta: float, sample_phi: bool = True) -> Dict[str, torch.Tensor]:
        phi = (self.gumbel_sigmoid_sample(self.phi_logits) if sample_phi else torch.sigmoid(self.phi_logits)).unsqueeze(0)
        x_exp = X.unsqueeze(1)
        mu_exp = self.q_mu.unsqueeze(0)
        logvar_exp = self.q_logvar.unsqueeze(0)

        log_prob_cluster = -0.5 * (math.log(2 * math.pi) + logvar_exp + (x_exp - mu_exp) ** 2 / torch.exp(logvar_exp))
        log_prob_bg = -0.5 * (
            math.log(2 * math.pi) + self.prior_logvar_0 + (x_exp - self.prior_mu_0) ** 2 / torch.exp(self.prior_logvar_0)
        )
...
class DIVIClustering:
    ...
    def _initialize_components(self, X_np: np.ndarray, K0: int):
        N, D = X_np.shape
        if K0 <= 1:
            return torch.mean(torch.tensor(X_np, dtype=torch.float32), dim=0, keepdim=True), torch.ones(1, D) * -1.0, torch.ones(1)
        ...
        for k in range(K0):
            Xk = X_np[labels == k]
            if Xk.shape[0] <= 1:
                var_k = X_np.var(axis=0) + 1e-3
            else:
                var_k = Xk.var(axis=0) + 1e-3
            logvars[k] = np.log(var_k)
        return centers, torch.tensor(logvars, dtype=torch.float32), torch.tensor(pis, dtype=torch.float32)
    ...
    def fit(self, X_np: np.ndarray, use_prior: int = 1) -> "DIVIClustering":
        ...
        init_means, init_logvar, init_pi = self._initialize_components(X_np, K0)
        ...
        for epoch in range(1, self.max_epochs + 1):
            ...
            # split intentionally disabled in fixed-K comparison mode
