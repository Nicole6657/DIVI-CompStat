...
def parse_args() -> argparse.Namespace:
    ...
    # Explicit overrides for split schedule / threshold
    parser.add_argument(
        "--tau-mult",
        type=float,
        default=1.0,
        help="Multiplier applied to DIVI auto split threshold.",
    )
    parser.add_argument(
        "--split-interval",
        type=int,
        default=None,
        help="Override split_interval from config/YAML.",
    )
    parser.add_argument(
        "--beta-mult",
        type=float,
        default=None,
        help="Optional override for beta_mult from config/YAML.",
    )
...
def resolved_divi_cfg(cfg: Dict[str, Any], args: argparse.Namespace, D: int) -> Dict[str, Any]:
    split_interval = int(args.split_interval) if args.split_interval is not None else int(cfg.get("split_interval", 80))
    beta_mult = float(args.beta_mult) if args.beta_mult is not None else float(cfg.get("beta_mult", 1.0))
    tau_auto = float(DIVIClustering.auto_split_threshold(D=D, sigma2=1.0))
    tau = float(args.tau_mult) * tau_auto
    return {
...
def run_divi_once(
    ...
    prior_mode: int,
    run_seed: int,
    divi_cfg: Dict[str, Any],
    phi_threshold: float,
    verbose: bool,
) -> Dict[str, Any]:
    set_global_seed(run_seed)
    model = DIVIClustering(
        split_threshold=float(divi_cfg["tau"]),
        split_interval=int(divi_cfg["split_interval"]),
        max_epochs=int(divi_cfg["max_epochs"]),
        lr=float(divi_cfg["lr"]),
        beta_mult=float(divi_cfg["beta_mult"]),
        temperature_start=float(divi_cfg["temp_start"]),
        temperature_end=float(divi_cfg["temp_end"]),
        verbose=verbose,
    )
...
def main() -> None:
    ...
    for scenario in args.scenarios:
        ...
        for N in args.N_list:
            for run_idx in range(args.n_runs):
                ...
                divi_cfg = resolved_divi_cfg(cfg=cfg, args=args, D=int(X.shape[1]))
                ...
                common = {
                    ...
                    "Tsplit": int(divi_cfg["split_interval"]),
                    "tau_mode": "manual_mult_auto",
                    "tau": float(divi_cfg["tau"]),
                    "tau_mult": float(divi_cfg["tau_mult"]),
                    "lr": float(divi_cfg["lr"]),
                    "temp_start": float(divi_cfg["temp_start"]),
                    "temp_end": float(divi_cfg["temp_end"]),
                    "max_epochs": int(divi_cfg["max_epochs"]),
                ...
                for prior_mode in args.prior_modes:
                    ...
                    record["experiment_id"] = make_experiment_id(
                        prefix=f"misspec_tuned{('_' + args.tag) if args.tag else ''}",
                    ...
                    try:
                        result = run_divi_once(
                            ...
                            run_seed=run_seed,
                            divi_cfg=divi_cfg,
                            phi_threshold=float(args.phi_threshold),
                            verbose=args.verbose,
                        )
                        record.update(result)
                        record["beta"] = result.get("beta", divi_cfg["beta_mult"] * len(X))
                        record["beta_mult"] = float(divi_cfg["beta_mult"])
                    ...
                    print(
                        ...
                        f"tau_mult={divi_cfg['tau_mult']:.2f} Tsplit={divi_cfg['split_interval']} "
    ...
    summary = summarize_results(
        runs_csv=runs_csv,
        group_cols=["dataset", "dataset_variant", "split_name", "method", "prior_mode", "N", "tau_mult", "Tsplit"],
        out_csv=summary_csv,
    )
    print("\nSaved:")
